...
@minilm_router.get("/db_status/", description="생성된 Vector DB 조회")
async def db_status():
    try:
        if pc.list_indexes():
...
@minilm_router.get("/db_dimensions/", description="현재 설정된 Vector DB dimensions 조회")
...
@minilm_router.post("/minilm_upload/", description="paraphrase-multilingual-MiniLM-L12-v2 모델 사용 Vector DB에 384차원 벡터 데이터 업로드")
async def upload_file(dbname:str,  file: UploadFile = File(...)):
    try:
        ...
        index.upsert(vectors=formatted_vectors, namespace="job4")
        return {"message": "Data uploaded successfully!"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@minilm_router.get("/minilm_jobsearch/", description="paraphrase-multilingual-MiniLM-L12-v2 모델 사용 질문에 대한 유사한 답변을 Vector DB에서 검색")
async def job_search(dbname:str, question: str):
    try:
        index = pc.Index(dbname)
...
async def similarity_verification(dbname:str, file: UploadFile = File(...)):
    try:
        index = pc.Index(dbname)
